...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (!strcmp(arg, "--matepair") || !strcmp(arg, "-mp")) {
      ...
      len_flag = true;
      if (read_len <= 0) {
        cerr << "Fatal Error: The read length must be a positive integer."
             << endl;
        arg_success = false;
      }
      ...
      i++;
      total_count = atof(argv[i]);
      n_count_flags++;
      total_flag = true;
      if (total_count <= 0) {
        cerr << "Fatal Error: The total reads must be positive." << endl;
        arg_success = false;
      }
      ...
      cc_flag = true;
      if (read_count < 0) {
        cerr << "Fatal Error: the read count must be a positive integer."
             << endl;
        arg_success = false;
      }
      ...
      mean_flag = true;
      if (mean < 0) {
        cerr << "Input Error: The mean fragment length must be a positive."
             << endl;
        arg_success = false;
      }
      ...
      if (min_qual_s < 0 || min_qual_s > max_qual) {
        cerr << "Input Error: The minimum quality score must be an integer "
                "in [0,"
             << max_qual << "]" << endl;
        arg_success = false;
      }
      ...
      if (max_qual_s <= 0 || max_qual_s > max_qual) {
        cerr << "Input Error: The quality score must be an integer in [1,"
             << max_qual << "]" << endl;
        arg_success = false;
      }
  ...
  if (no_ALN && !sam_out) {
    cerr << "Warning: your simulation will not output any ALN or SAM file "
            "with your parameter settings!"
         << endl;
  }

  // Make sure the minimum requirements to run were met if the help tag was
  // not given
  if (help_flag) {
    arg_success = false;
  } else if (n_count_flags != 1) {
    cerr << "Error: please use only one of the parameters: read count "
            "(-c),fold coverage (-f) or total count (-t)."
         << endl
         << endl;
    exit(1);
  } else if (!in_flag || !out_flag || !len_flag) {
    arg_success = false;
    //	cerr << "Fatal Error: An input-file, output-file prefix, read
    // length, and fold coverage must be specified." << endl << endl;
  }
  ...
  if (mean_flag && sDev_flag) {
    ...
    if (mean >= 2000) {
      ...
      cerr << "Warning: a mate-pair simulation may be not appropriate for "
              "DNA fragment size < 2000bp"
           << endl;
    }
    num = "1";
  }
  ...
  if (is_pairend_read) {
    ...
    if (mean_flag && sDev_flag) {
      ...
      if (art::gaussain_mean <= read_len) {
        cerr << "Fatal Error: The read length must be shorter than the mean "
                "fragment length specified."
             << endl;
        exit(1);
      }
    } else if (!amplicon) {
      cerr << "Fatal Error: A mean fragment length and a standard deviation "
              "must be specified."
           << endl
           << endl;
      exit(1);
  ...
  if (!arg_success) {
    ...
    cout << "art_illumina [options] -ss <sequencing_system> -sam -i "
            "<seq_ref_file> -l <read_length> -f <fold_coverage> -o "
            "<outfile_prefix>"
         << endl;
    cout << "art_illumina [options] -ss <sequencing_system> -sam -i "
            "<seq_ref_file> -l <read_length> -c <num_reads_per_sequence> -o "
            "<outfile_prefix>"
         << endl;
    cout << "art_illumina [options] -ss <sequencing_system> -sam -i "
            "<seq_ref_file> -l <read_length> -f <fold_coverage> -m "
            "<mean_fragsize> -s <std_fragsize> -o <outfile_prefix>"
         << endl;
    cout << "art_illumina [options] -ss <sequencing_system> -sam -i "
            "<seq_ref_file> -l <read_length> -c <num_reads_per_sequence> -m "
            "<mean_fragsize> -s <std_fragsize> -o <outfile_prefix>"
         << endl
         << endl;
    ...
    cout << "  -c   --rcount   number of reads/read pairs to be generated "
            "per sequence/amplicon (not be used together with -f/--fcov)"
         << endl;
    cout << "  -t   --total    total number of reads/read pairs to be generated, will be split by bp proportion of each seq" << endl;
    cout << "  -d   --id       the prefix identification tag for read ID"
         << endl;
    cout << "  -ef  --errfree  indicate to generate the zero sequencing "
            "errors SAM file as well the regular one"
         << endl;
    cout << "                  NOTE: the reads in the zero-error SAM file "
            "have the same alignment positions"
         << endl;
    cout << "                  as those in the regular SAM file, but have no "
            "sequencing errors"
         << endl;
    cout << "  -f   --fcov     the fold of read coverage to be simulated or "
            "number of reads/read pairs generated for each amplicon"
         << endl;
    cout << "  -h   --help     print out usage information" << endl;
    cout << "  -i   --in       the filename of input DNA/RNA reference" << endl;
    cout << "  -ir  --insRate  the first-read insertion rate (default: "
            "0.00009)"
         << endl;
    cout << "  -ir2 --insRate2 the second-read insertion rate (default: "
            "0.00015)"
         << endl;
    cout << "  -dr  --delRate  the first-read deletion rate (default:  "
            "0.00011)"
         << endl;
    cout << "  -dr2 --delRate2 the second-read deletion rate (default: "
            "0.00023)"
         << endl;
    cout << "  -k   --maxIndel the maximum total number of insertion and "
            "deletion per read (default: up to read length)"
         << endl;
    cout << "  -l   --len      the length of reads to be simulated" << endl;
    cout << "  -m   --mflen    the mean size of DNA/RNA fragments for "
            "paired-end simulations"
         << endl;
    cout << "  -mp  --matepair indicate a mate-pair read simulation" << endl;
    cout << "  -M  --cigarM    indicate to use CIGAR 'M' instead of '=/X' "
            "for alignment match/mismatch"
         << endl;
    cout << "  -nf  --maskN    the cutoff frequency of 'N' in a window size "
            "of the read length for masking genomic regions"
         << endl;
    cout << "                  NOTE: default: '-nf 1' to mask all regions "
            "with 'N'. Use '-nf 0' to turn off masking"
         << endl;
    cout << "  -na  --noALN    do not output ALN alignment file" << endl;
    cout << "  -o   --out      the prefix of output filename" << endl;
    cout << "  -p   --paired   indicate a paired-end read simulation or to "
            "generate reads from both ends of amplicons"
         << endl;
    cout << "                  NOTE: art will automatically switch to a "
            "mate-pair simulation if the given mean fragment size >= 2000"
         << endl;
    ...
    cout << "  -qs  --qShift   the amount to shift every first-read quality "
            "score by "
         << endl;
    cout << "  -qs2 --qShift2  the amount to shift every second-read quality "
            "score by"
         << endl;
    cout << "                  NOTE: For -qs/-qs2 option, a positive number "
            "will shift up quality scores (the max is 93) "
         << endl;
    cout << "                  that reduce substitution sequencing errors "
            "and a negative number will shift down "
         << endl;
    cout << "                  quality scores that increase sequencing "
            "errors. If shifting scores by x, the error"
         << endl;
    cout << "                  rate will be 1/(10^(x/10)) of the default "
            "profile."
         << endl;
    //	cout << "  -qo  --qOrig    indicate to output the original
    // quality scores regardless the original scores were shifted or not" <<
    // endl;
    cout << "  -rs  --rndSeed  the seed for random number generator "
            "(default: system time in second)"
         << endl;
    cout << "                  NOTE: using a fixed seed to generate two "
            "identical datasets from different runs"
         << endl;
    cout << "  -s   --sdev     the standard deviation of DNA/RNA fragment "
            "size for paired-end simulations."
         << endl;
    cout << "  -sam --samout   indicate to generate SAM alignment file" << endl;
    cout << "  -sp  --sepProf  indicate to use separate quality profiles for "
            "different bases (ATGC)"
         << endl;
    cout << "  -ss  --seqSys   The name of Illumina sequencing system of the "
            "built-in profile used for simulation"
         << endl;
    cout << "       NOTE: sequencing system ID names are:" << endl;
    cout << "            GA1 - GenomeAnalyzer I (36bp,44bp), GA2 - "
            "GenomeAnalyzer II (50bp, 75bp)"
         << endl;
    cout << "           HS10 - HiSeq 1000 (100bp),          HS20 - HiSeq "
            "2000 (100bp),      HS25 - HiSeq 2500 (125bp, 150bp)"
         << endl;
    cout << "           HSXn - HiSeqX PCR free (150bp),     HSXt - HiSeqX "
            "TruSeq (150bp),   MinS - MiniSeq TruSeq (50bp)"
         << endl;
    cout << "           MSv1 - MiSeq v1 (250bp),            MSv3 - MiSeq v3 "
            "(250bp),        NS50 - NextSeq500 v2 (75bp)"
         << endl;

    cout << "===== NOTES =====" << endl << endl;
    cout << "* ART by default selects a built-in quality score profile "
            "according to the read length specified for the run."
         << endl
         << endl;
    cout << "* For single-end simulation, ART requires input sequence file, "
            "output file prefix, read length, and read count/fold coverage."
         << endl
         << endl;
    ;
    //	cout << "  Example: art --in reference_DNA.fa --out sim1 --len
    // 35 --fcov 2 -sam" << endl << endl;;
    cout << "* For paired-end simulation (except for amplicon sequencing), "
            "ART also requires the parameter values of"
         << endl;
    cout << "  the mean and standard deviation of DNA/RNA fragment lengths"
         << endl
         << endl;
    //	cout << "  Example: art --paired --in reference_DNA.fa --out
    // sim2 --len 35 --fcov 2 --mflen 200 --sdev 3.5 -sam" << endl << endl;

    cout << "===== EXAMPLES =====" << endl << endl;
    cout << " 1) single-end read simulation" << endl;
    cout << " 	art_illumina -ss HS25 -sam -i reference.fa -l 150 -f 10 "
            "-o single_dat"
         << endl
         << endl;
    cout << " 2) paired-end read simulation" << endl;
    cout << "       art_illumina -ss HS25 -sam -i reference.fa -p -l 150 -f "
            "20 -m 200 -s 10 -o paired_dat"
         << endl
         << endl;
    cout << " 3) mate-pair read simulation" << endl;
    cout << "       art_illumina -ss HS10 -sam -i reference.fa -mp -l 100 -f "
            "20 -m 2500 -s 50 -o matepair_dat"
         << endl
         << endl;
    cout << " 4) amplicon sequencing simulation with 5' end single-end reads "
         << endl;
    cout << " 	art_illumina -ss GA2 -amp -sam -na -i amp_reference.fa "
            "-l 50 -f 10 -o amplicon_5end_dat"
         << endl
         << endl;
    cout << " 5) amplicon sequencing simulation with paired-end reads" << endl;
    cout << "       art_illumina -ss GA2 -amp -p -sam -na -i "
            "amp_reference.fa -l 50 -f 10 -o amplicon_pair_dat"
         << endl
         << endl;
    cout << " 6) amplicon sequencing simulation with matepair reads" << endl;
    cout << "       art_illumina -ss MSv1 -amp -mp -sam -na -i "
            "amp_reference.fa -l 150 -f 10 -o amplicon_mate_dat"
         << endl
         << endl;
    cout << " 7) generate an extra SAM file with zero-sequencing errors for "
            "a paired-end read simulation"
         << endl;
    cout << "       art_illumina -ss HSXn -ef -i reference.fa -p -l 150 -f "
            "20 -m 200 -s 10 -o paired_twosam_dat"
         << endl
         << endl;
    cout << " 8) reduce the substitution error rate to one 10th of the "
            "default profile"
         << endl;
    cout << "       art_illumina -i reference.fa -qs 10 -qs2 10 -l 50 -f 10 "
            "-p -m 500 -s 10 -sam -o reduce_error"
         << endl
         << endl;
    cout << " 9) turn off the masking of genomic regions with unknown "
            "nucleotides 'N'"
         << endl;
    cout << "       art_illumina -ss HS20 -nf 0  -sam -i reference.fa -p -l "
            "100 -f 20 -m 200 -s 10 -o paired_nomask"
         << endl
         << endl;
    cout << " 10) masking genomic regions with >=5 'N's within the read "
            "length 50"
         << endl;
    cout << "       art_illumina -ss HSXt -nf 5 -sam -i reference.fa -p -l "
            "150 -f 20 -m 200 -s 10 -o paired_maskN5"
         << endl
         << endl;
  ...
  if (max_num_n > read_len) {
    cerr << "Error: the cutoff frequency of 'N' for maksing genome region: "
         << max_num_n << " > the read length (" << read_len << ")" << endl;
    exit(1);
  }
  if (!fixed_seed) {
  ...
  if (sam_out) {
    ...
    if (!SAMFILE.is_open()) {
      cerr << "Can not open output file: " << samfile << endl;
      exit(0);
    }
    if (err_free_sam) {
      SAMFILE_EF.open(samfile_ef.c_str(), ios::binary);
      if (!SAMFILE_EF.is_open()) {
        cerr << "Can not open output file: " << samfile_ef << endl;
        exit(0);
      }
    }
  }
  ...
  if (!FQFILE.is_open()) {
    cerr << "Can not open output file: " << fqfile << endl;
    exit(0);
  }
  ofstream ALNFILE;
  if (!no_ALN) {
    ...
    if (!ALNFILE.is_open()) {
      cerr << "Can not open output file: " << alnfasta << endl;
      exit(0);
    }
  ...
  if (qual_file1.empty() && !seqsys.empty()) {
    ...
    if (!seqsys.empty()) {
      cout << "Warnning: ART ignores the specified sequencing system as a "
              "customized quality profile provided.";
    }
  }
  ...
  if (read_len > profile_size) {
    if (profile_size == 0) {
      cerr << "Fatal Error: " << qual_file1 << ", is not a valid profile."
           << endl
           << endl;
    } else {
      cerr << "Fatal Error: The read length, " << read_len
  ...
  if ((read_len > profile_size_2) && is_pairend_read) {
    if (profile_size_2 == 0) {
      if (!seqsys.empty())
        cerr << "Error: the built-in " << seqsys
             << "  sequencing system supports only single-end sequencing"
             << endl
             << endl;
      else
        cerr << "Fatal Error: " << qual_file2 << ", is not a valid profile."
             << endl
             << endl;
    } else {
      cerr << "Fatal Error: The read length, " << read_len
  ...
  if (is_pairend_read && profile_size != profile_size_2) {
    cerr << "Fatal Error: The first profile read length, "
         << qdist.qual_dist_first.size()
         << ", does not match the second profile read length, "
         << qdist.qual_dist_second.size() << "." << endl
         << endl;
    exit(1);
  }
  ...
  if (!sep_flag) {
    for (size_t i = 0; i < qdist.qual_dist_first.size(); i++) {
      for (map<unsigned int, unsigned short>::iterator it =
               qdist.qual_dist_first[i].begin();
           it != qdist.qual_dist_first[i].end(); it++) {
        if (q_shift_up != 0) {
          if (q_shift_up < 0 && (-q_shift_up > it->second)) {
            it->second = min_qual_s;
          } else {
            it->second += q_shift_up;
            if (it->second > max_qual_s)
              it->second = max_qual_s;
          }
        }
        if (it->second < min_qual_s)
          it->second = min_qual_s;
        if (it->second > max_qual_s)
          it->second = max_qual_s;
      }
    ...
    for (size_t i = 0; i < qdist.qual_dist_second.size(); i++) {
      for (map<unsigned int, unsigned short>::iterator it =
               qdist.qual_dist_second[i].begin();
           it != qdist.qual_dist_second[i].end(); it++) {
        if (q_shift_up_2 != 0) {
          if (q_shift_up_2 < 0 && (-q_shift_up_2 > it->second)) {
            it->second = 0;
          } else {
            it->second += q_shift_up_2;
            if (it->second > max_qual_s)
              it->second = max_qual_s;
          }
        }
    ...
    for (size_t i = 0; i < qdist.a_qual_dist_first.size(); i++) {
      for (map<unsigned int, unsigned short>::iterator it =
               qdist.a_qual_dist_first[i].begin();
           it != qdist.a_qual_dist_first[i].end(); it++) {
        if (q_shift_up != 0) {
          if (q_shift_up < 0 && (-q_shift_up > it->second)) {
            it->second = 0;
          } else {
    ...
    for (size_t i = 0; i < qdist.a_qual_dist_second.size(); i++) {
      for (map<unsigned int, unsigned short>::iterator it =
               qdist.a_qual_dist_second[i].begin();
           it != qdist.a_qual_dist_second[i].end(); it++) {
        if (q_shift_up_2 != 0) {
          if (q_shift_up_2 < 0 && (-q_shift_up_2 > it->second)) {
            it->second = 0;
          } else {
    ...
    for (size_t i = 0; i < qdist.c_qual_dist_first.size(); i++) {
      for (map<unsigned int, unsigned short>::iterator it =
               qdist.c_qual_dist_first[i].begin();
           it != qdist.c_qual_dist_first[i].end(); it++) {
        if (q_shift_up != 0) {
          if (q_shift_up < 0 && (-q_shift_up > it->second)) {
            it->second = 0;
          } else {
    ...
    for (size_t i = 0; i < qdist.c_qual_dist_second.size(); i++) {
      for (map<unsigned int, unsigned short>::iterator it =
               qdist.c_qual_dist_second[i].begin();
           it != qdist.c_qual_dist_second[i].end(); it++) {
        if (q_shift_up_2 != 0) {
          if (q_shift_up_2 < 0 && (-q_shift_up_2 > it->second)) {
            it->second = 0;
          } else {
    ...
    for (size_t i = 0; i < qdist.g_qual_dist_first.size(); i++) {
      for (map<unsigned int, unsigned short>::iterator it =
               qdist.g_qual_dist_first[i].begin();
           it != qdist.g_qual_dist_first[i].end(); it++) {
        if (q_shift_up != 0) {
          if (q_shift_up < 0 && (-q_shift_up > it->second)) {
            it->second = 0;
          } else {
    ...
    for (size_t i = 0; i < qdist.g_qual_dist_second.size(); i++) {
      for (map<unsigned int, unsigned short>::iterator it =
               qdist.g_qual_dist_second[i].begin();
           it != qdist.g_qual_dist_second[i].end(); it++) {
        if (q_shift_up_2 != 0) {
          if (q_shift_up_2 < 0 && (-q_shift_up_2 > it->second)) {
            it->second = 0;
          } else {
    ...
    for (size_t i = 0; i < qdist.t_qual_dist_first.size(); i++) {
      for (map<unsigned int, unsigned short>::iterator it =
               qdist.t_qual_dist_first[i].begin();
           it != qdist.t_qual_dist_first[i].end(); it++) {
        if (q_shift_up != 0) {
          if (q_shift_up < 0 && (-q_shift_up > it->second)) {
            it->second = 0;
          } else {
    ...
    for (size_t i = 0; i < qdist.t_qual_dist_second.size(); i++) {
      for (map<unsigned int, unsigned short>::iterator it =
               qdist.t_qual_dist_second[i].begin();
           it != qdist.t_qual_dist_second[i].end(); it++) {
        if (q_shift_up_2 != 0) {
          if (q_shift_up_2 < 0 && (-q_shift_up_2 > it->second)) {
            it->second = 0;
          } else {
  ...
  //    void set_rate(int read_len, double p, vector <double>& rate, int
  //    max_num=0, double cdf_cutoff=0.999999){
  ...
  if (is_pairend_read) {
    ...
    //        if(!SEQFILE2.is_open()) { cout<<"can not open output file:
    //        "<<seqfasta2<<endl; exit(0); }

    //        ofstream QUALFILE2(qualfasta2.c_str(),ios::binary);
    //        if(!QUALFILE2.is_open()) { cout<<"can not open output file:
    //        "<<qualfasta2<<endl; exit(0); }

    ofstream FQFILE2(fqfile2.c_str(), ios::binary);
    if (!FQFILE2.is_open()) {
      cerr << "Can not open output file: " << fqfile2 << endl;
      exit(0);
    }
    ofstream ALNFILE2;
    if (!no_ALN) {
      ...
      if (!ALNFILE2.is_open()) {
        cerr << "Can not open output file: " << alnfasta2 << endl;
        exit(0);
      }
    ...
    map<string, size_t> ref_lens;
    size_t total_len = 0;
    map<string, int> ref_num_reads;
    while (seq_reader.next_seq(id, a_art.ref_seq)) {

      std::replace(a_art.ref_seq.begin(), a_art.ref_seq.end(), 'U', 'T'); // replace U with T
      //            size_t p1=id.find_first_of(' '); if(p1==string::npos)
      //            p1=10; size_t p2=id.find_first_of('\t');
      //            if(p2==string::npos) p2=10;            p1=p1<p2?p1:p2;
      //            id=id.substr(0,p1);
      istringstream isID;
      isID.str(id);
      isID >> id;
      // id = id.substr(0, len_ref_id);
      num_seq++;
      ...
      if(a_art.read_len>a_art.ref_seq.length()){
        cerr << "Warning: the reference sequence " << id << " (length "
             << a_art.ref_seq.size()
             << "bps ) is skipped as it < the defined read length (" << read_len
             << " bps)" << endl;
        continue;
      }
      size_t ref_len = a_art.ref_seq.length();
      total_len += ref_len;
      ref_lens[id] = ref_len;
      // cout << id << "\t" << ref_len << endl;
    }
    int prop_total = 0;
    map<string, double> remainders;
    for (auto it = ref_lens.begin(); it != ref_lens.end(); ++it) {
      // cout << it->first << "\t" << it->second << "\t" << endl;

      double prop = (double)it->second / total_len * total_count;
      double remainder = prop - (int) prop;
      remainders[it->first] = remainder;
      ref_num_reads[it->first] = (int) ((double)it->second / total_len * total_count);
      prop_total += ref_num_reads[it->first];
    }
    if (prop_total < total_count) {
      vector<pair<string, double>> pairs;
      for (auto itr = remainders.begin(); itr != remainders.end(); ++itr)
        pairs.push_back(*itr);

      sort(pairs.begin(), pairs.end(), [=](std::pair<string, double>& a, std::pair<string, double>& b)
           {
             return a.second > b.second;
           }
           );
      for (auto p: pairs) {
        ref_num_reads[p.first]++;
        prop_total++;
        if (prop_total == total_count) {
          break;
        }
    ...
    // cout << "prop total" << "\t" << prop_total << "\t" << endl;
    seq_reader.restart();
    ...
    while (seq_reader.next_seq(id, a_art.ref_seq)) {
      ...
      if (cc_flag) {
        ...
        t_num_read = 2 * ref_num_reads[id];
        // cout << id << "\t" << t_num_read << endl;
      } else {
        t_num_read = static_cast<unsigned long>(a_art.ref_seq.size() / read_len * x_fold);
        if (amplicon) {
          t_num_read = 2 * (long)x_fold;
        }
      }
      while (t_num_read > 0) {
        //              osID<<num_seq<<fixed<<setfill('0')<< setw(10)<<
        //              t_num_read;
        ...
        if (is_matepair) {
          if (amplicon)
            has_reads = a_art.next_matepair_ampread_indel_cmp(a_read, a_read_2);
          else
            has_reads = a_art.next_pair_read_indel_mate(a_read, a_read_2);
        } else {
          if (amplicon)
            has_reads = a_art.next_pair_ampread_indel_cmp(a_read, a_read_2);
          else
        ...
        if (mask_n) {
          if (a_read.is_plus_strand) {
            ...
            if (a_art.masked_pos.count(a_read.bpos) > 0 ||
                a_art.masked_pos.count(bpos2) > 0) {
              t_num_read -= 2;
              continue;
            }
          } else {
            ...
            if (a_art.masked_pos.count(bpos1) > 0 ||
                a_art.masked_pos.count(a_read_2.bpos) > 0) {
              t_num_read -= 2;
              continue;
            }
          }
        ...
        FQFILE2 << "@" << read_id_2 << endl
                << a_read_2.seq_read << endl
                << "+" << endl;
        ...
        if (sam_out) {
          ...
          if (err_free_sam) {
            ...
            sR.cigar = p_cigar;
            for (size_t k = 0; k < sR.qual.size(); k++)
              sR.qual[k] = max_q_c;
            sR2.seq = a_read_2.seq_ref;
            sR2.cigar = p_cigar;
            for (size_t k = 0; k < sR2.qual.size(); k++)
              sR2.qual[k] = max_q_c;
            if (a_read.is_plus_strand)
              sR2.reverse_comp();
            else
    ...
    while (seq_reader.next_seq(id, a_art.ref_seq)) {
      std::replace(a_art.ref_seq.begin(), a_art.ref_seq.end(), 'U',
                   'T'); // replace U with T
      istringstream isID;
      isID.str(id);
      isID >> id;
      ...
      if (cc_flag) {
        ...
        t_num_read = static_cast<unsigned long>(a_art.ref_seq.size() /
                                                read_len * x_fold);
        if (amplicon) {
          t_num_read = (long)x_fold;
        }
      }
      if (mask_n) {
        a_art.mask_n_region(max_num_n);
      ...
      while (t_num_read > 0) {
        ...
        if (sam_out) {
          ...
          if (a_read.is_plus_strand) {
            ...
            sR.flag = 0x10;
            sR.pos = a_art.ref_seq.size() -
                     (a_read.bpos + a_read.seq_read.size() - 1);
            sR.reverse_comp();
          }
          sR.getCigar(aln_ref, aln_read, use_cigarM);
          sR.printRead(SAMFILE);
  ...
  if (!is_pairend_read) {
    if (amplicon) {
      cout << "              Amplicon 5'-end sequencing simulation" << endl
           << endl;
    } else {
    ...
    if (amplicon) {
      cout << "             Amplicon matepair sequencing simulation" << endl
           << endl;
    } else {
    ...
    if (amplicon) {
      cout << "             Amplicon paired-end sequencing simulation" << endl
           << endl;
    } else {
  ...
  if (!show_flag) {
    ...
    if (is_pairend_read) {
      if (first_qual) {
        cout << "\tFirst Read:   " << qual_file1.c_str() << " (user's profile)"
             << endl;
      } else if (!qdist.ssystem.empty()) {
        cout << "\tFirst Read:   " << qdist.ssystem << " Length "
             << profile_size << " R1"
             << " (built-in profile) " << endl;
      } else {
        cout << "\tFirst Read:   "
             << " EMP" << profile_size << "R1"
             << " (built-in profile) " << endl;
      ...
      if (second_qual) {
        cout << "\tSecond Read:  " << qual_file2.c_str() << " (user's profile)"
             << endl
             << endl;
      } else if (!qdist.ssystem.empty()) {
        cout << "\tFirst Read:   " << qdist.ssystem << " Length "
        ...
        cout << "\tSecond Read:  "
             << " EMP" << profile_size_2 << "R2"
             << " (built-in profile) " << endl
             << endl;
      }
    } else {
      if (first_qual) {
        cout << "\t" << qual_file1.c_str() << " (user's profile)" << endl
             << endl;
      } else if (!qdist.ssystem.empty()) {
        cout << "\tFirst Read:   " << qdist.ssystem << " Length "
             << profile_size << " R1"
